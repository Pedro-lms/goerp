...
import (
	...
	"github.com/Pedro-lmso-erp/erp/src/tools/strutils"
...
const (
	...
	// Models is the PackageType for the erp/models package
	Models
)
...
// GetModulePackages returns a slice of PackageInfo for packages that are erp modules, that is:
// - A package that declares a "MODULE_NAME" constant
// - A package that is in a subdirectory of a package
// Also returns the 'erp/models' package since all models are initialized there
func GetModulePackages(packs []*packages.Package) []*ModuleInfo {
	...
	// and we check for 'erp/models' package
...
func defaultFields(name string) map[string]FieldASTData {
	...
	case "ModelMixin":
		res["erpExternalID"] = FieldASTData{
			Name:        "erpExternalID",
