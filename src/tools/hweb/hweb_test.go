...
import (
	...
	"github.com/Pedro-lmso-erp/erp/src/tools/xmlutils"
...
{% endmacro %}{% set __erp_template_name = "subtemplate" %}{% include __erp_template_name with var1 = valueInner %}
{% endwith %}
`)
		So(transpileVariables(doc.ChildElements()), ShouldBeNil)
		resXML, err = doc.WriteToString()
		...
		<div style="display: none" class="oe_secondary_menu" data-menu-parent="{{ menu.id }}">
			{% for menu in menu.children %}
				<div class="oe_secondary_menu_section" data-menu-xmlid="{{ menu.xmlid }}">
					...
					{% if not menu.children %}{% with _0 = null %}{% macro _0() %}{% endmacro %}{% set __erp_template_name = "web.menu_link" %}{% include __erp_template_name  %}
{% endwith %}{% endif %}
				</div>
				{% with _0 = null %}{% macro _0() %}{% endmacro %}{% set __erp_template_name = "web.menu_secondary_submenu" %}{% include __erp_template_name  %}
{% endwith %}
			{% endfor %}
		</div>
	{% endfor %}
