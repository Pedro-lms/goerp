...
func transpileCalls(elts []*etree.Element) error {
	for _, elt := range elts {
		...
		// We set a __erp_template_name variable in order to lazy load the template (avoids infinite recursion)
		endWithCD := etree.NewCharData(fmt.Sprintf("{%% set __erp_template_name = \"%s\" %%}{%% include __erp_template_name %s %%}\n{%% endwith %%}", fea.Value, with))
