...
func LoadCSVDataFile(fileName string) {
	...
	err = ExecuteInNewEnvironment(security.SuperUserID, func(env Environment) {
		...
		for {
			...
			delete(values, "id")
			values["erp_external_id"] = externalID
			values["erp_version"] = version
			// We deliberately call Search directly without Call so as not to be polluted by Search overrides
			// such as "Active test".
			rec := rc.Search(rc.Model().Field(rc.model.FieldName("erpExternalID")).Equals(externalID)).Limit(1)
			switch {
			case rec.Len() == 0:
				...
				rc.Call("Create", vals)
			case rec.Len() == 1:
				if version > rec.Get(rec.model.FieldName("erpVersion")).(int) || update {
...
func getRecordValuesMap(headers []string, modelName string, record []string, env Environment, line int, fileName string) FieldMap {
	...
	for i := 0; i < len(headers); i++ {
		...
		case fi.fieldType.IsFKRelationType():
			...
			if record[i] != "" {
				relRC := env.Pool(fi.relatedModelName).Search(fi.relatedModel.Field(fi.relatedModel.FieldName("erpExternalID")).Equals(record[i]))
				if relRC.Len() != 1 {
		...
		case fi.fieldType == fieldtype.Many2Many:
			...
			relRC := env.Pool(fi.relatedModelName).Search(fi.relatedModel.Field(fi.relatedModel.FieldName("erpExternalID")).In(ids))
			val = relRC
