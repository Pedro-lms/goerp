...
const (
	...
	ManualModel
	// SystemModel is a model that is used internally by the erp Framework
	SystemModel
)
...
func commonMixinDefaultGet(rc *RecordCollection) *ModelData {
	res := rc.getDefaults(rc.Env().Context().GetBool("erp_ignore_computed_defaults"))
	return res
}
...
func commonMixinOnChange(rc *RecordCollection, params OnchangeParams) OnchangeResult {
	...
	err := SimulateInNewEnvironment(rc.Env().Uid(), func(env Environment) {
		...
		if id, _ := nbutils.CastToInteger(data.Get(ID)); id != 0 {
			...
			rs = rs.WithContext("erp_onchange_origin", rs.First().Wrap())
			rs.WithContext("erp_force_compute_write", true).update(data)
		} else {
			rs = rc.WithEnv(env).WithContext("erp_force_compute_write", true).create(data)
		}
		// Set inverse fields
		...
		for len(todo) > 0 {
			...
			if fnct != "" {
				...
				rrs.WithContext("erp_force_compute_write", true).Call("Write", vals)
			}
			// Warning
