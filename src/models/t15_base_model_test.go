	...
	Convey("Testing base model methods", t, func() {
		So(SimulateInNewEnvironment(security.SuperUserID, func(env Environment) {
			...
			Convey("DefaultGet", func() {
				...
				So(defaults.FieldMap, ShouldContainKey, "erp_external_id")
...
`)
			...
			Convey("GetRecord", func() {
				So(env.Pool("User").Call("GetRecord", userJane.Get(erpExternalID)).(RecordSet).Collection().Equals(userJane), ShouldBeTrue)
			})
