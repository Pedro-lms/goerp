...
import (
	...
	"github.com/Pedro-lmso-erp/erp/src/tools/typesutils"
)
...
func (rc *RecordCollection) processTriggers(keys FieldNames) {
	if rc.Env().Context().GetBool("erp_no_recompute_stored_fields") {
		return
	}
...
func (rc *RecordCollection) applyMethod(methodName string) {
	for _, rec := range rc.Records() {
		...
		if doUpdate {
			rec.WithContext("erp_force_compute_write", true).Call("Write", data)
		}
	}
}
...
func (rc *RecordCollection) processInverseMethods(data RecordData) {
	...
	for _, fieldName := range md.Underlying().Keys() {
		...
		if !fi.isComputedField() || rc.Env().Context().GetBool("erp_force_compute_write") {
			continue
		}
		if !md.Has(fName) {
			continue
		...
		val := md.Get(fName)
		if fi.inverse == "" {
			if typesutils.IsZero(val) || rc.Env().Context().GetBool("erp_ignore_computed_fields") {
				continue
			}
