...
var moduleCmd = &cobra.Command{
	...
	Long:  `erp utilities for module development.`,
}

var moduleInitCmd = &cobra.Command{
	Use:   "init MODULE_PATH",
	Short: "Initialize a module",
	Long: `Initialize and scaffold a new module in the current directory with the given path (e.g. github.com/myuser/my-erp-module).
...
For local only modules (i.e. modules tied to a project), use 'erp module new' from the project directory instead.`,
	Run: func(cmd *cobra.Command, args []string) {
		...
		// Create the 000erp.go file
		data := struct {
			ModuleName string
		}{
			ModuleName: path.Base(modulePath),
		}
		if err := writeFileFromTemplate("000erp.go", erpGoTmpl, data); err != nil {
			fmt.Println(err)
			os.Exit(1)
		}
...
The current directory must be an erp project directory.

If you plan to make a module and distribute it on its own, you should create a new directory and run 'erp module init' inside instead.`,
	Run: func(cmd *cobra.Command, args []string) {
		if len(args) == 0 {
		...
		if res, err := c.Output(); err != nil || string(res) != "main" {
			fmt.Println("You must call erp module new from a project directory.")
			fmt.Println(err)
			os.Exit(1)
		}
		...
		if err := writeFileFromTemplate(filepath.Join(projectPath, "000erp.go"), erpGoTmpl, data); err != nil {
			fmt.Println(err)
			os.Exit(1)
		}
		// Create standard directories
